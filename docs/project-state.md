...
Goal: compare `Clean Removal` against `Persistent Corpses` on the same fixed 2D map and enemy setup, testing whether visible dead remains make revisited spaces feel more historical, memorable, and changed by prior Player actions.

Ready 2D SVG assets exist under Prototype #004 `assets/` and should be used directly. Corpse collision, loot, decay, pollution, scavengers, ecology, respawn and generalized frameworks are out of scope.
