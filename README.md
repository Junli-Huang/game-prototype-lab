...
V0.2.6 — Equipment Mobility Comparison。[Prototype #001：Backpack Lab](prototypes/001_spatial_backpack/README.md) 保留 EXP-001 MAYBE、EXP-003 TESTING / Untested、EXP-048 MAYBE，并已实现 EXP-049 Fixed vs Movable Equipment — TESTING / Untested。[Prototype #002：Enemy Respawn Lab](prototypes/002_campfire_respawn/README.md) 的 EXP-007 / EXP-008 为 MAYBE / Stop。
...
当前没有 Active implementation task。EXP-049 已完成技术与 Pages 验收，下一步是 Player 对比试玩 Movable / Locked Required Equipment；不要从 Backlog 自行猜测下一任务。

## Experiment Backlog

[查看全部 49 个实验课题](docs/experiment-backlog.md)。当前关键状态：EXP-001、EXP-048 为 MAYBE；EXP-003、EXP-049 为 TESTING / Untested；EXP-007 / EXP-008 为 MAYBE / Stop。
