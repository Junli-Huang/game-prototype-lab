...
V0.1 — Game Prototype Lab Skeleton。当前只有 Launcher 空列表，没有正式玩法 Prototype。
...
1. Designer 提出一个 Gameplay Hypothesis，只问一个核心玩法问题。
...
```text
Idea → Build → Play → Evaluate → Kill / Iterate / Promote
```

状态：IDEA / BUILDING / TESTING / INTERESTING / MAYBE / DEAD / PROMOTED。详见 [Philosophy](docs/philosophy.md)。有趣的实验小规模继续验证，再做 Vertical Slice，最后迁移独立游戏仓库；原实验保留记录。
